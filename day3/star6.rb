...
$cache_to_line.values.combination(2) do |claim1, claim2|
  claims = claim1.intersect_rect(claim2)
  if claims.empty?
    $colide[claim1.id] ||= false
    $colide[claim2.id] ||= false
  else
    $colide[claim1.id] = true
    $colide[claim2.id] = true

  end

  claims.each do |point|
    $overlap[point.x] ||= []
